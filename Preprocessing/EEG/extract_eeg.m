...
downSampleFactor = timestamps_temp.clockrate/1000;
...
newFs = length(eeg_temp{1}.data)/(endtime-starttime); %fix timestamps bug
fprintf('%s\n',newFs);
load(sprintf('%s/%stimestamps%02d.mat',animaldir,prefix,day)); %timestamps file needs to store all epochs
timestamps{day}{epoch} = timestamps_temp.fields.data(1):1/newFs:timestamps_temp.fields.data(end); %add new epoch
savename = sprintf('%s/%stimestamps%02d.mat',animaldir,prefix,day);
save(savename,'-v6','timestamps');
